...
use super::{
    BinOp, Decl, Defer, Direction, Expr, Interface, Interner, Param, Position, Schema, SchemaBody,
    Sym, Transition, Type, UnOp, Variant,
};


// ============================================================================
// Lowering: Decl<String> -> Decl<Sym>
// ============================================================================
...
fn lower_defer(d: Defer<String>, interner: &mut Interner) -> Defer<Sym> {
    let mut pos_map: BTreeMap<Sym, Sym> = BTreeMap::new();
    ...
    for (k, inner) in d.dir_map {
        ...
        let mut new_inner: BTreeMap<Sym, Sym> = BTreeMap::new();
        for (tk, tv) in inner {
...
fn lower_schema(s: Schema<String>, interner: &mut Interner) -> Schema<Sym> {
    let body = match s.body {
        ...
        SchemaBody::Sum(variants) => SchemaBody::Sum(
            variants
                .into_iter()
                .map(|v| lower_variant(v, interner))
                .collect(),
        ),
    };
    Schema {
